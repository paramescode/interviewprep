...
public class TaskList {
    ...
    public static int[] solution(int[] data, int n) {
        ...
        Map<Integer, List<Integer>> count = new LinkedHashMap<>();
        for(int index=0; index < data.length ; index ++){
            if(count.get(data[index]) == null){
                List<Integer> list = new ArrayList<>();
                list.add(index);
                count.put(data[index], list);
            }else{
                count.get(data[index]).add(index);
            }
        }
        ArrayList<Integer> resultList = new ArrayList<>();
        for(Integer key : count.keySet()){
            if(count.get(key).size() > n){
                resultList.addAll(count.get(key));
        ...
        int [] result = new int[data.length - resultList.size()];
        int index =0;
    ...
    public static void main(String[] args) {
        int [] re = solution(new int[]{5, 10, 15, 10, 7}, 1);
        for (Integer v: re
             ) {
